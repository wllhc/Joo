...
--
-- 表的结构 `pin_photo` 照片信息主表
--

DROP TABLE IF EXISTS `pin_photo`;
CREATE TABLE `pin_photo` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `cate_id` smallint(4) unsigned DEFAULT NULL COMMENT '所属分类',
  `uid` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '照片所属用户ID',
  `uname` varchar(20) NOT NULL COMMENT '照片所属用户名',
  `title` varchar(255) DEFAULT NULL COMMENT '照片主标题',
  `intro` varchar(255) NOT NULL COMMENT '照片描述（副标题）',
  `img` varchar(255) DEFAULT NULL COMMENT '照片存放路径（可以是绝对或相对路径）',
  `price` decimal(10,2) DEFAULT NULL COMMENT '价格',
  `rates` float(8,2) NOT NULL COMMENT '佣金比率xxx.xx%，暂留，以备扩展',
  `type` tinyint(1) NOT NULL DEFAULT '1' COMMENT '1:普通照片,2:手机照片',
  ...
  `tag_cache` text NOT NULL COMMENT '标签cache',
...
--
-- 表的结构 `pin_photo_attr` 照片属性表
-- 用来存放照片 浏览量、热度、喜欢数、评论数、等需频繁更新的数据
--

DROP TABLE IF EXISTS `pin_photo_attr`;
CREATE TABLE `pin_photo_attr` (
...
--
-- 表的结构 `pin_photo_cate` 照片分类表
--

DROP TABLE IF EXISTS `pin_photo_cate`;
CREATE TABLE `pin_photo_cate` (
  `id` smallint(4) unsigned NOT NULL AUTO_INCREMENT,
  `name` varchar(50) NOT NULL,
  `tags` varchar(50) NOT NULL COMMENT '分类包含的标签',
  `pid` smallint(4) unsigned NOT NULL COMMENT '父分类',
  `spid` varchar(50) NOT NULL,
  `img` varchar(255) NOT NULL COMMENT '分类封面图',
  `fcolor` varchar(10) NOT NULL COMMENT '分类标题color',
  `remark` text NOT NULL,
...
--
-- 表的结构 `pin_photo_cate_tag` 照片分类标签关联表
--

DROP TABLE IF EXISTS `pin_photo_cate_tag`;
CREATE TABLE `pin_photo_cate_tag` (
  `cate_id` smallint(4) unsigned NOT NULL,
...
--
-- 表的结构 `pin_photo_comment` 照片评论表
--

DROP TABLE IF EXISTS `pin_photo_comment`;
CREATE TABLE `pin_photo_comment` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
...
--
-- 表的结构 `pin_photo_like` 用户照片评分表（取代喜欢功能）
--

DROP TABLE IF EXISTS `pin_photo_mark`;
CREATE TABLE `pin_photo_like` (
  `photo_id` int(10) unsigned NOT NULL,
  `uid` int(10) unsigned NOT NULL,
  `mark` tinyint(2) unsigned NOT NULL COMMENT '评分，1-5分'
  `add_time` int(10) NOT NULL,
...
DROP TABLE IF EXISTS `pin_photo_tag`;
CREATE TABLE `pin_photo_tag` (
  `photo_id` int(10) unsigned NOT NULL,
  `tag_id` int(10) unsigned NOT NULL,
  UNIQUE KEY `photo_id` (`photo_id`,`tag_id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8;
...
--
-- 表的结构 `pin_team_member` 团队成员
--

DROP TABLE IF EXISTS `pin_team_member`;
CREATE TABLE `pin_team_member` (
  `id` int(10) unsigned NOT NULL,
  `name` varchar(20) NOT NULL COMMENT '名字',
  `nick_name` varchar(20) NOT NULL COMMENT '昵称',
  `age` tinyint(3) NOT NULL COMMENT '年龄',
  `hometown` varchar(64) NOT NULL COMMENT '家乡',
  `role` varchar(100) NOT NULL COMMENT '职位，职称',
  `desc` varchar(500) NOT NULL COMMENT '个人简介，座右铭等',
