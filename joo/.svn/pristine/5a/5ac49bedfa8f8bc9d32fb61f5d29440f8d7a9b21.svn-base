...
class itemAction extends frontendAction
{
    ...
    public function _initialize()
    {
        parent::_initialize();
        //访问者控制
        if (!$this->visitor->is_login && in_array(ACTION_NAME, array('share_item', 'fetch_item', 'publish_item', 'like', 'unlike', 'no_sell', 'delete', 'comment'))) {
    ...
    public function index()
    {
        ...
        !$id && $this->_404();
        $item_mod = M('item');
        $item = $item_mod->field('id,title,img,uid,uname,intro,price,Model,FocalLength,ShutterSpeedValue,ApertureValue,ISOSpeedRatings,DateTimeOriginal,Width,Height,url,hits,likes,comments,laud,freedown,tag_cache,seo_title,seo_keys,seo_desc,add_time')
        ...
        foreach ($maylike_list as $key => $val) {
            ...
            $maylike_list[$key]['list'] = $item_tag_mod->field('i.id,i.img,i.MimeType,i.uid,i.extension,i.intro,' . $item_tag_table . '.tag_id')->where(array($item_tag_table . '.tag_id' => $key, 'i.id' => array('neq', $id)))->join($db_pre . 'item i ON i.id = ' . $item_tag_table . '.item_id')->order('i.id DESC')->limit(9)->select();
        ...
        $item['thumb'] = $this->getThumbInfo($item);
    ...
    public function addcart()
    {
        if ($this->isAjax()) {
            ...
            if (!in_array($item_id, $_SESSION['cart_item'])) {
                $_SESSION['cart_item'][] = $item_id;
                $this->ajaxReturn(1, '加入成功！', '');
            } else {
                $this->ajaxReturn(0, '已经在购物车中了哦！', '');
            }
        }
    }
    ...
    public function cart()
    {
        ...
        $item_mod = M('item');
        $map['id'] = array('in', $cart_item);
        $item = $item_mod->where($map)->select();
        $this->assign('item', $item);
        $this->display();
    }
    ...
    public function laud()
    {
        ...
        $uname = $this->visitor->info['username'];
        $item['uid'] == $uid && $this->ajaxReturn(0, L('like_own')); //自己的商品
        $laud_mod = M('item_laud');
        //是否已经喜欢过
        $is_liked = $laud_mod->where(array('item_id' => $item['id'], 'uid' => $uid))->count();
        $is_liked && $this->ajaxReturn(0, L('您已经赞过了！'));
        if ($laud_mod->add(array('item_id' => $item['id'], 'uid' => $uid, 'add_time' => time()))) {
            //增加商品赞数
            $item_mod->where(array('id' => $id))->setInc('laud');
            //增加用户被赞数
            M('user')->where(array('id' => $item['uid']))->setInc('laud');
            //增加专辑喜欢
            //$aid && M('album')->where(array('id' => $aid))->setInc('laud');
            //添加喜欢钩子
            //$tag_arg = array('uid' => $uid, 'uname' => $uname, 'action' => 'likeitem');
            //tag('likeitem_end', $tag_arg);
            $this->ajaxReturn(1, L('已赞成功！'));
        } else {
            $this->ajaxReturn(0, L('赞失败！'));
        }
    }
     ...
     * 返回status(todo)
     */
    public function unlaud()
    {
        $id = $this->_get('id', 'intval');
        !$id && $this->ajaxReturn(0, L('invalid_item'));
        $uid = $this->visitor->info['id'];
        $laud_mod = M('item_laud');
        if ($laud_mod->where(array('uid' => $uid, 'item_id' => $id))->delete()) {
            //赞数不减少~
    ...
    public function freedown()
    {
        ...
        $uname = $this->visitor->info['username'];
        $item['uid'] == $uid && $this->ajaxReturn(0, L('like_own')); //自己的商品
        $down_mod = M('item_freedown');
        //是否已经喜欢过
        $is_down = $down_mod->where(array('item_id' => $item['id'], 'uid' => $uid))->count();
        $is_down && $this->ajaxReturn(1, L('下载成功！'));
        if ($down_mod->add(array('item_id' => $item['id'], 'uid' => $uid, 'add_time' => time()))) {
            //增加商品赞数
            $item_mod->where(array('id' => $id))->setInc('freedown');
            //增加用户被赞数
           // M('user')->where(array('id' => $item['uid']))->setInc('laud');
            //增加专辑喜欢
            //$aid && M('album')->where(array('id' => $aid))->setInc('laud');
            ...
            //$tag_arg = array('uid' => $uid, 'uname' => $uname, 'action' => 'likeitem');
            //tag('likeitem_end', $tag_arg);
            $this->ajaxReturn(1, L('下载成功！'));
        } else {
            $this->ajaxReturn(0, L('下载失败！'));
        }
    }
    ...
    public function set_index()
    {
        if ($this->isAjax()) {
            ...
            !$id && $this->ajaxReturn(0, L('invalid_item'));
            !$album_id && $this->ajaxReturn(0, '没有相应的专辑！');
            $album_item = M('album_item');
            // 先取消掉所有的专辑的封面再设置
            $res = $album_item->where('album_id=' . $album_id)->setField('is_index', 0);
            $result = $album_item->where('album_id=' . $album_id . ' AND item_id=' . $id)->setField('is_index', 1);
            if ($result) {
                $this->ajaxReturn(1, '设置封面成功！');
            } else {
                $this->ajaxReturn(0, '设置封面失败！');
            }
        }
    }
     ...
     * 商品上架下架
     */
    public function no_sell()
    {
        if ($this->isAjax()) {
            $id = $this->_post('id', 'intval');
            !$id && $this->ajaxReturn(0, L('invalid_item'));
            $item = M('item');
            $is_sell = $item->where('id=' . $id)->getField('is_sell');

            if ($is_sell) {
                $item->where('id=' . $id)->setField('is_sell', 0);
                $this->ajaxReturn(1, '商品已经下架！');
            } else {
                $item->where('id=' . $id)->setField('is_sell', 1);
                $this->ajaxReturn(1, '商品上架成功！');
            }
        }
    }
