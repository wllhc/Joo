...
class spaceAction extends frontendAction
{
    ...
    public function album()
    {
        ...
        if ($type == 'followed') {
            ...
            $pager = $this->_pager($count, $pagesize);
            $album_list = $album_mod->field('id,uid,title,items,cover_cache,add_time')->where($map)->order($sort_order)->limit($pager->firstRow . ',' . $pager->listRows)->select();
        }
        $album_id = array();
        foreach ($album_list as $key => $val) {
            $album_id[] = $val['id'];
            $album_list[$key]['cover'] = unserialize($val['cover_cache']);
            $album_list[$key]['items_display'] = ($album_list[$key]['items'] - 7) >= 0 ? $album_list[$key]['items'] : 0;
            $album_list[$key]['add_time'] = date('Y-m-d', $album_list[$key]['add_time']);
        }

        // 专辑对应的图片
        $item = M('album_item');
        if (!empty($album_id)) {
            $m['album_id'] = array('in', $album_id);
            $list = $item->where($m)->select();
        } else {
            $list = $item->select();
        }
        $ar = array();
