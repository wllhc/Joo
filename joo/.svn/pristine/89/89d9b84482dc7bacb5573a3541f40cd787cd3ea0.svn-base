...
class frontendAction extends baseAction
{
     ...
     * @return array
     */
    protected function get_hots()
    {
        $item_mod = M('item');
        $hots = array();
        $info = $this->visitor->info;
        $hots['hits'] = $item_mod->where('uid=' . $info['id'])->sum('hits');
        $hots['likes'] = $item_mod->where('uid=' . $info['id'])->sum('likes');
        $hots['laud'] = $item_mod->where('uid=' . $info['id'])->sum('laud');
        $hots['comments'] = $item_mod->where('uid=' . $info['id'])->sum('comments');
        $hots['count'] = ($hots['hits'] + $hots['likes'] + $hots['laud'] + $hots['comments']);
        return $hots;
    }
    ...
    public function waterfall($where = array(), $order = 'id DESC', $field = '', $page_max = '', $target = '')
    {
        ...
        foreach ($item_list as $key => $val) {
            ...
            // 获取缩略图
            $mime = explode(".", $val['img']);
            $file_name = $val['uid'] . '_' . $val['id'] . '.' . $mime[count($mime) - 1];
            $item_list[$key]['thumb'] = $this->getThumbInfo($val);
        }
    ...
    public function wall_ajax($where = array(), $order = 'id DESC', $field = '')
    {
        ...
        foreach ($item_list as $key => $val) {
            ...
            $mime = explode(".", $val['img']);
            $file_name = $val['uid'] . '_' . $val['id'] . '.' . $mime[count($mime) - 1];
            $item_list['thumb'] = $this->getThumbInfo($val);
        }
        $this->assign('item_list', $item_list);
