...
function get_thumb($img, $size = 'small') {
    return $img[$size];
 ...
 * @return string
 */
function get_upload_file_name($file_name) {
    $mime = get_file_ext($file_name);

    $str = hash('ripemd160', md5($file_name));
    $rand_path = rand(1000, 9999) . '/' . mt_rand(0,10) . '/';
    $path = './data/photos/' . $rand_path;

    if(!file_exists($path)) {
        mkdir($path, 0777,true);
    }

    return $rand_path . $str.'.'.$mime;
}