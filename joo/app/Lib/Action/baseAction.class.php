...
class baseAction extends Action
{
     ...
     *  获取图片缩略图路径
     *
     * @param Array $item 图片数组
     * @returns Array     
     */                   
    protected function getThumbInfo($item)
    {
        $result = array();
        if (isset($item['img']) && isset($item['uid']) && isset($item['id'])) {
            $mime = explode(".", $item['img']);
            $file_name = $item['uid'] . '_' . $item['id'] . '.' . $mime[count($mime) - 1];
            $result = array(
              'small' => 'data/thumb/' . $this->__thumb_size['small'] . $file_name,
              'big' => 'data/thumb/' . $this->__thumb_size['big'] . $file_name,
              );
        }
