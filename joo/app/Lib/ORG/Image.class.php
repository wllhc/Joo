...
class Image
{
    ...
    static function cutImage($image, $cutImage, $type = '', $x, $y, $width, $height, $newWidth, $newHeight)
    {
        ...
        // 输出图要从哪边开始x, y , 原始图要从哪边开始 x, y , 要输多大 w, h(resize) , 要抓多大 w, h
        imagecopyresampled($newim, $im, 0, 0, $x, $y, $newWidth, $newHeight, $width, $height);

        /* 保存图片 */
        $imageFun = 'image' . ($type == 'jpg' ? 'jpeg' : $type);
        $imageFun($newim, $cutImage);
        /* 资源回收 */
        imagedestroy($newim);
        imagedestroy($im);
        return $cutImage;
     ...
     * @internal param string $size 图像大小 width,height 或者 array(width,height)
     * @internal param string $font 字体信息 fontface,fontsize 或者 array(fontface,fontsize)
     * @return string
     */
