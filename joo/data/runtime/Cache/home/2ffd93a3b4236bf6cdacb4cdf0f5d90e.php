    ...
    <dl>
        ...
        <div class="setting_box fl">
            <div id="setting_form" class="account_bind">
                <div class="account_bind_title"><?php echo L('bind_title');?></div>
                <ul class="bind_logo">
                    <?php if(is_array($oauth_list)): $i = 0; $__LIST__ = $oauth_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><li>
                        <div class="icon"><img src="__STATIC__/images/oauth/<?php echo ($val["code"]); ?>/bind.png" /></div>
                        <div class="bind_web" style="height:70px;">
                            <p class="unbind"><?php echo ($val["name"]); ?> <?php if($val["isbind"] == 1): echo L('binded'); echo ($val["name"]); else: ?>(<?php echo L('nobind');?>)<?php endif; ?></p>
                            <?php if($val["isbind"] == 1): ?><p class="mt5"><a href="<?php echo U('oauth/index', array('type'=>'unbind', 'mod'=>$val['code']));?>" class="clr6"><?php echo L('unbind');?></a></p>
                            <?php else: ?>
                            <p class="bind_result"><a href="<?php echo U('oauth/index', array('type'=>'bind', 'mod'=>$val['code']));?>" class="btn"><?php echo L('gobind');?></a></p><?php endif; ?>
                        </div>
                    </li><?php endforeach; endif; else: echo "" ;endif; ?>
                </ul>
            </div>
