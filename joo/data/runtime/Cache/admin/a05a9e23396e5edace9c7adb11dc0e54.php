...
<!--数据库恢复-->
<div class="subnav">
    <h1 class="title_2 line_x"><?php echo L('database_restore');?></h1>
</div>
<div class="pad_lr_10">
    ...
    <form action="<?php echo U('backup/import');?>" method="post">
    	<table width="100%" cellspacing="0" class="treeTable">
            <thead>
                <tr>
                    <th align="left"><?php echo L('backup_name');?></th>
                    <th><?php echo L('backup_size');?></th>
            ...
            <?php if(is_array($backups)): $i = 0; $__LIST__ = $backups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr class="collapsed">
                ...
                <td align="center"><?php echo ($val["total_size"]); ?>kb</td>
                <td align="center"><?php echo ($val["date_str"]); ?></td>
                <td align="center">
                    <a href="<?php echo u('backup/del_backup', array('backup'=>$val['name']));?>"><?php echo L('delete');?></a> | 
                    <a href="<?php echo u('backup/import', array('backup'=>$val['name']));?>" ><?php echo L('import');?></a>
                </td>
            </tr>
            <?php if(is_array($val['vols'])): $i = 0; $__LIST__ = $val['vols'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr parent="<?php echo ($val["name"]); ?>" class="hidden">
                <td><?php echo ($vol["file"]); ?></td>
                <td align="center"><?php echo ($vol["size"]); ?>kb</td>
                <td align="center"><?php echo ($val["date_str"]); ?></td>
                <td align="center">
                    <a href="<?php echo u('backup/download', array('backup'=>$val['name'], 'file'=>$vol['file']));?>"><?php echo L('download');?></a>
                </td>
