    ...
    $.pinphp.item = {
        ...
        like: function(){
            ...
            $(s.like_btn).live('click', function(){
                ...
                var self = $(this),
                    nb = self.siblings('.J_like_n').find('strong'),
        ...
        laud: function(){
            var s = $.pinphp.item.settings;
            $(s.laud_btn).live('click', function(){
                if(!$.pinphp.dialog.islogin()) return !1;
                var self = $(this),
                    nb = self.siblings('.J_laud_n').find('strong'),
                    id = self.attr('data-id'),
                    aid = self.attr('data-aid'),
                    n = parseInt(nb.html());
                $.getJSON(PINER.root + '/?m=item&a=laud', {id:id, aid:aid}, function(result){
                    if(result.status == 1){
                        //赞成功
                        nb.text(n+1).show();
                        $.pinphp.tip({content:result.msg});
        ...
        freedown: function(){
            var s = $.pinphp.item.settings;
            $(s.freedown_btn).live('click', function(){
                if(!$.pinphp.dialog.islogin()) return !1;
                var self = $(this),
                    nb = self.siblings('.J_laud_n').find('strong'),
                    id = self.attr('data-id'),
                    aid = self.attr('data-aid'),
                    n = parseInt(nb.html());
                $.getJSON(PINER.root + '/?m=item&a=freedown', {id:id, aid:aid}, function(result){
                    if(result.status == 1){
                        //赞成功
                        nb.text(n+1).show();
                        $.pinphp.tip({content:result.msg});
        ...
        share: function(){
            ...
            $(s.share_btn).live('click', function(){
                ...
                setTimeout(function() { window.location = PINER.root + '/?m=upload&a=index&aid=' + aid; }, 0);
                /*
                if(!$.pinphp.dialog.islogin()) return !1;
                var aid = $(this).attr('data-aid');
